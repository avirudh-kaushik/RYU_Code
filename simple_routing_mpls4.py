...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.attribute_band_dir = []
        self.attribute_del_dir = []
        self.counter = 0
    ...
    def optimum_path(self,m_path,delay,band,path):
        ...
        ret_list = []
        for paths in m_path:
            if paths[1] < band and paths[0] > delay:

                m_path[i] = (delay,band,path)
                check = 1
                i = i+1


            elif paths[1] <= band or paths[0] >= delay:
                flag = 1
                i = i+1

        if flag == 1:
            m_path.append((delay,band,path))

        temp_set = set(m_path)
        ret_list = list(temp_set)
    ...
    def dijkstra(self,edges, f, t):
        g = defaultdict(list)
        m_path = []
        for l,r,b,d in edges:
            g[l].append((d,b,r))

        q, seen = [(0,0,f,())], set()
        while q:
            (delay,band,v1,path) = heappop(q)
            if v1 not in seen:
                seen.add(v1)
            path = (v1, path)
            if v1 == t:
                if not m_path:
                    ...
                    m_path = self.optimum_path(m_path,delay,band,path)

            for d,b,v2 in g.get(v1, ()):
                if v2 not in seen:
                    if b > band and band !=0:
                        b = band

                    heappush(q, (delay+d,b,v2, path))
        ...
        for i in out:
            ...
            while len(i)>1:
                ...
                if check == 0:
                    ...
                    aux.append(i[0])
                    i = i[1]
            aux.remove(data['delay'])
            aux.remove(data['bandwidth'])
            aux.reverse()
            data['path']=aux
            ans.append(aux)
        return ans
    ...
    def mplsHandler(self,msg):
        ...
        if self.host_to_switch[dst] == dpid:
            ...
            match = parser.OFPMatch(eth_type = ethtype,mpls_label = mpls_proto.label)
        ...
        if self.counter == 0:
            ...
            for line in f:
                ...
                if s[0] != '\n' and "*" not in s[0]:
                    ...
                    #print s
                    self.attribute_band_dir.append(s[2])
                    self.attribute_del_dir.append(s[3])
                    if s[0] not in self.dir_of_switches:
                        self.dir_of_switches.append(s[0])
                    if s[1] not in self.dir_of_switches:
                        self.dir_of_switches.append(s[1])
            ...
            temp1 = 0
            for x in edges:
                temp = (int(self.attribute_band_dir[temp1]),)
                #self.edge_list.append(x + temp)
                temp2 = (int(self.attribute_del_dir[temp1]),)
                self.edge_list.append(x + temp + temp2)
